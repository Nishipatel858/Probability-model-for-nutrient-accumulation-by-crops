...
mean=[20.75 23.75 23.5];
sd=[14.22146	9.429563	5.744563];

fr_A_intersect_N=[0 1 1.5];
fr_A_intersect_P=[5 4 3.75];
fr_A_intersect_S=[0 0.8 1.5];
fr_A_intersect_K=[2 3 1.7];

fr_B_intersect_N=[6 5 4.9];
fr_B_intersect_P=[4 8 3];
...
for i=1:3
    Pr_soybean_N(i)=fr_N(i)/Total_Plot;
    Pr_soybean_P(i)=fr_P(i)/Total_Plot;
    Pr_soybean_S(i)=fr_S(i)/Total_Plot;
    Pr_soybean_K(i)=fr_K(i)/Total_Plot;
    ...
    Pr_A_given_N(i) = Pr_A_intersect_N(i)/Pr_soybean_N(i);
    Pr_A_given_P(i) = Pr_A_intersect_P(i)/Pr_soybean_P(i);
    Pr_A_given_S(i) = Pr_A_intersect_S(i)/Pr_soybean_S(i);
    Pr_A_given_K(i) = Pr_A_intersect_K(i)/Pr_soybean_K(i);
    
    Pr_N_intersect_A_given_N(i)=Pr_soybean_N(i)*Pr_A_given_N(i);
    Pr_P_intersect_A_given_P(i)=Pr_soybean_P(i)*Pr_A_given_P(i);
    Pr_S_intersect_A_given_S(i)=Pr_soybean_S(i)*Pr_A_given_S(i);
    Pr_K_intersect_A_given_K(i)=Pr_soybean_K(i)*Pr_A_given_K(i);
    ...
    Pr_B_given_N(i) = Pr_B_intersect_N(i)/Pr_soybean_N(i);
    Pr_B_given_P(i) = Pr_B_intersect_P(i)/Pr_soybean_P(i);
    Pr_B_given_S(i) = Pr_B_intersect_S(i)/Pr_soybean_S(i);
    Pr_B_given_K(i) = Pr_B_intersect_K(i)/Pr_soybean_K(i);
    
    Pr_N_intersect_B_given_N(i)=Pr_soybean_N(i)*Pr_B_given_N(i);
    Pr_P_intersect_B_given_P(i)=Pr_soybean_P(i)*Pr_B_given_P(i);
    Pr_S_intersect_B_given_S(i)=Pr_soybean_S(i)*Pr_B_given_S(i);
    Pr_K_intersect_B_given_K(i)=Pr_soybean_K(i)*Pr_B_given_K(i);
...
set(gca,'XTickLabel',{'2001','2009','2015'})
...
set(gca,'XTickLabel',{'2001-2009','2009-2015','2001-2015','2001-2009-2015'})
